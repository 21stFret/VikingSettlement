...
| Date | Bug | Summary |
|------|-----|---------|
| 2026-08-23 | B54 — Pause blocked after a cutscene plays to completion (works fine if skipped) | `PlayCutscene` disables the `Player` input action map (where `Pause` lives) and enables `CutScene` for the duration. `StopCutscene` (used by skip) restores both via `DelayFrameForInput`, but `CompleteCutscene` (natural end, all actions finished) never did — `Player` stayed disabled indefinitely after any cutscene that played out instead of being skipped. Fixed by calling the same `DelayFrameForInput()` restore from `CompleteCutscene` too. |
| 2026-08-23 | B53 — Jarl death on a raid fired succession in the raid scene, bogus-Game-Overed before the real (post-return) succession could run | `JarlManager` is scene-local like `SettlementManager`, but a raid scene also has its own live instance (`RaidManager.AssignPlayerControl` uses it for player control). `Villager.Die()` called `JarlManager.Instance.OnCurrentJarlDied()` unconditionally on `isJarl`; in the raid scene `SettlementManager.Instance` is null, so `GetSuccessionCandidates()`/`HandleNoHeirs()` always concluded "no heirs" and called `TriggerGameOver()` immediately, racing the correct `RaidManager.ApplyPendingResults()` casualty-replay path that already existed for this exact case. Fixed by gating the trigger on `!wasOnRaid` (captured before `base.Die()`, since its `OnDeath` cascade flips `RaidManager.IsOnRaid` false mid-call via `Defeat()`/`EndRaid()`). Also added `Villager.SetDeathCauseOverride()` so a raid/simulated casualty replayed via `Die()` on a restored instance reports the correct death cause (Combat) to `JarlManager` for the Valkyrie's Favour/Gefjon's Blessing choice, instead of guessing from that instance's own never-actually-damaged flags. |
